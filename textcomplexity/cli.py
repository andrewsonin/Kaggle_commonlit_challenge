...
def convert_to_conll(text = 'Say hello to my little NN model', language = 'en', model_dir = '.', nlp = None):


    #stanza.download('en')

    if nlp is None:
        nlp = stanza.Pipeline(language, processors="tokenize,mwt,pos,lemma,depparse", model_dir= model_dir)
    else: nlp = nlp
    ...
    for sentence in conll:
        ...
        #     conll_new.append("\t".join(token))

        # text = io.StringIO(text)
        conll_new.append("\n".join(("\t".join(token) for token in sentence)))
        conll_new.append("\n\n")

    conll_new = io.StringIO(''.join(conll_new))

    return conll_new

import io

def text_to_fuatures(text ='I love you. I love you!!!',
                     sur = None, sent=None, dep = None, const = None,
                     input_format = 'conllu', window_size = 5, all_measures = True,
                     ignore_punct = False, output_format = 'json', lang = None,
                     model_dir = '.', nlp = None):

    text = convert_to_conll(text, model_dir=model_dir, nlp = nlp)

    if not any((sur, sent, dep, const)):
        sur = True
        sent = True
        dep = True
        const = True

    punct_tags = None #TODO you can change this arg
    if punct_tags is None:
        punct_tags = ["PUNCT"]
    punct_tags = set(punct_tags)
    all_results = {}
    new_all_results = {'number_text': []}


    tokens, tagged, graphs, ps_trees = None, None, None, None
    if input_format == "conllu":
        tokens, tagged, graphs = zip(
            *conllu.read_conllu_sentences(text, ignore_punct=ignore_punct, punct_tags=punct_tags))
        tokens = list(itertools.chain.from_iterable(tokens))

    elif input_format == "tsv":

        tokens, tagged, graphs, ps_trees = zip(
            *custom_tsv.read_tsv_sentences(text, ignore_punct=ignore_punct, punct_tags=punct_tags))
        tokens = list(itertools.chain.from_iterable(tokens))

    results = []
    if sur and tokens is not None:
        results.extend(surface_based(tokens, window_size, all_measures))
    if sent and tagged is not None:
        results.extend(sentence_based(tagged, punct_tags))
    if dep and graphs is not None:
        results.extend(dependency_based(graphs))
    if const and ps_trees is not None:
        results.extend(constituency_based(ps_trees, lang))
    all_results['fix_name'] = {}

    new_all_results['number_text'].append('fix_name')

    for r in results: # TODO fix this by adding all info
        if r.name in new_all_results.keys():
            new_all_results[r.name].append(r.value)
        else:
            new_all_results[r.name] = [r.value]

    # for r in results:
    #     all_results['fix_name'][r.name] = {"value": r.value}
    #     if r.stdev is not None:
    #         all_results['fix_name'][r.name]["stdev"] = r.stdev
    #     if r.length is not None:
    #         all_results['fix_name'][r.name]["length"] = r.length
    #         all_results['fix_name'][r.name]["length stdev"] = r.length_stdev
...
def main():
    ...
    for i, f in enumerate(args.TEXT):
        ...
        new_all_results['number_text'].append(f.name)
    ...
    new_all_results_df.to_csv('data_test.csv')
    return new_all_results_df

    #
    # if args.output_format == "json":
    #
    #     #print(args.TEXT['name'])
    #     print('Дамплю в json!')
